...
import { normalizeRehearsalInventory, runSyntheticRehearsal } from "./production-shaped-rehearsal-core.mjs";
...
const rehearsalInventory = normalizeRehearsalInventory(inventory);
...
assert.match(sql.stdout, /Clean bootstrap passed: 76 ordered migrations and 21 AWS target overlays/);
...
const evidence = { format: "tracepoint-production-shaped-rehearsal/v1", generatedAt: new Date().toISOString(), sourceInventorySha256: rehearsalInventory.contentSha256, sourceScale: model.scale, lineage: { productionSource: 60, authoritativeSource: 76, sourceDeltasAfterCopy: 16, awsOverlaysAfterCopy: 21, targetTotal: 97 }, database: { ...model.database, cleanBootstrap: true, dumpRestoreReconciled: true, dumpRestoreMs: restoreMs, bootstrapAndRestoreWallMs: sqlMs, retryContract: "single-transaction restore; exact anchor-only or full-content resume; partial/conflicting data fails closed" }, identity: model.identity, storage: model.storage, rollback: { preWriteBridgeRestore: "allowed", postWriteBridgeRestore: "freeze-and-reconcile" }, timings: { syntheticContractMs: modelMs, totalMs: Date.now() - startedAt }, privacy: { syntheticOnly: true, emailsEmitted: false, userIdsEmitted: false, objectKeysEmitted: false, recordContentsEmitted: false } };
