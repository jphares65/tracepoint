...
function source(overrides: Partial<ArmoryReadDataSource> = {}): ArmoryReadDataSource { return { listActiveAssignments: async () => result(), listFirearms: async () => result(), listActiveMembers: async () => result(), listProfiles: async () => result(), listInspections: async () => result(), ...overrides }; }
...
test("preserves exact Supabase query contracts", async () => {
  ...
  const adapter = new SupabaseArmoryReadDataSource({ from: (table: string) => (calls.push(["from", table]), query) });
