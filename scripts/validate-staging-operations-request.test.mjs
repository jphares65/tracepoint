...
const context={branch:'refs/heads/main',head:'c'.repeat(40),parent:'a'.repeat(40),changedFiles:['.github/staging-operations.json'],imageIsAncestor:true};
...
test('production, management, arbitrary commands and mixed-source pushes are rejected',()=>{
 ...
 for(const patch of [{branch:'refs/heads/codex/aws-staging-readiness-20260902'},{imageIsAncestor:false},{changedFiles:['.github/staging-operations.json','src/app/page.tsx']}])assert.throws(()=>validateOperationsRequest(request,{...context,...patch}));
});
