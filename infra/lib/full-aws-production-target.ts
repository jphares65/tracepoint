...
export function validateFullAwsProductionTarget(value:FullAwsProductionTarget,{offline=false,now=Date.now()}={}){
 ...
 if(!new RegExp(`^arn:aws:acm:us-east-1:${value.account}:certificate/[0-9a-f-]{36}$`).test(value.certificateArn)||!/^[0-9a-f]{40}-aws-native$/.test(value.imageTag))throw Error("Production certificate and provider-qualified immutable image required");
