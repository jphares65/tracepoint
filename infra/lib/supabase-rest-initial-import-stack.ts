...
export interface SupabaseRestInitialImportStackProps extends cdk.StackProps {
  runId: string; authorizationReference: string; commit: string; imageDigest: string;
  repositoryName: string; clusterName: string; vpcId: string; publicSubnetIds: string[];
  databaseSecurityGroupId: string; sourceSecretArn: string; targetSecretArn: string;
}
...
export class SupabaseRestInitialImportStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: SupabaseRestInitialImportStackProps) {
    ...
    const rdsHost='tracepoint-production.c8r4sgs089tu.us-east-1.rds.amazonaws.com';
    ...
    if(!/^[0-9a-f]{40}$/.test(props.commit)||!/^sha256:[0-9a-f]{64}$/.test(props.imageDigest)||props.repositoryName!=='tracepoint-production'||props.clusterName!=='tracepoint-production') throw new Error('Exact immutable migration image context is required');
    if(props.vpcId!=='vpc-04accb4047a914176'||props.publicSubnetIds.length!==2||new Set(props.publicSubnetIds).size!==2||!props.publicSubnetIds.every(value=>['subnet-0f4cbed3e60d90bfc','subnet-0a117bec5cb98607f'].includes(value))||props.databaseSecurityGroupId!=='sg-096e4787eae992cf4'||props.sourceSecretArn!==sourceSecretArn||props.targetSecretArn!==targetSecretArn) throw new Error('Only reviewed migration network, source, and target are permitted');
    ...
    const common=(logGroup:logs.LogGroup, importMode:'database'|'objects', roleArn:string, secrets:{name:string,valueFrom:string}[], taskRoleArn?:string):ecs.CfnTaskDefinitionProps=>({family:`tracepoint-production-rest-${importMode}-initial-import-${runSuffix}`,requiresCompatibilities:['FARGATE'],networkMode:'awsvpc',cpu:'512',memory:'1024',executionRoleArn:roleArn,...(taskRoleArn?{taskRoleArn}:{}),containerDefinitions:[{name:importMode==='database'?'rest-rds-import':'rest-object-copy',image:`${repository.repositoryUri}@${props.imageDigest}`,essential:true,readonlyRootFilesystem:true,user:'node',logConfiguration:{logDriver:'awslogs',options:{'awslogs-group':logGroup.logGroupName,'awslogs-region':region,'awslogs-stream-prefix':importMode}},environment:[{name:'TRACEPOINT_MIGRATION_RUN_ID',value:props.runId},{name:'TRACEPOINT_MIGRATION_AUTHORIZATION_REFERENCE',value:props.authorizationReference},{name:'TRACEPOINT_EXPECTED_AWS_ACCOUNT',value:account},{name:'SOURCE_SUPABASE_REST_SECRET_ARN',value:sourceSecretArn},{name:'TRACEPOINT_REST_IMPORT_MODE',value:importMode},{name:'TRACEPOINT_SOURCE_COMMIT',value:props.commit},...(importMode==='database'?[{name:'TARGET_DATABASE_SECRET_ARN',value:targetSecretArn},{name:'TARGET_PGHOST',value:rdsHost},{name:'TARGET_PGDATABASE',value:'tracepoint'}]:[{name:'TRACEPOINT_TARGET_BUCKET',value:bucket}])],secrets}]});
    this.databaseTaskDefinition=new ecs.CfnTaskDefinition(this,'DatabaseTaskDefinition',common(dbLogs,'database',dbRole.roleArn,[{name:'SOURCE_SUPABASE_REST_SECRET_JSON',valueFrom:sourceSecretArn},{name:'TARGET_DATABASE_SECRET_JSON',valueFrom:targetSecretArn}]));
