...
function targetClient(target, ca, application_name) {
  ...
  client.connect = async () => {
    ...
    const expectedAddresses = (await lookup(FINAL_CLEAN_TARGET_HOST, { all: true, family: 4 })).map(({ address }) => address);
    const identity = (await client.query("select current_database() as database, inet_server_addr()::text as server_address, inet_server_port()::int as server_port")).rows[0];
    assert.equal(identity.database, "tracepoint", "MIGRATION_TARGET_DATABASE_MISMATCH");
    assert.ok(expectedAddresses.includes(identity.server_address), "MIGRATION_TARGET_SERVER_MISMATCH");
    client.migrationTargetAttestation = { configuredHost: FINAL_CLEAN_TARGET_HOST, database: identity.database, serverAddress: identity.server_address, serverPort: identity.server_port, tlsVerified: true };
  };
  return client;
}
