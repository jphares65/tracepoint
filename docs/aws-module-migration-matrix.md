...
| `GET /api/qualifications` imported history | typed `QualificationHistoryRepository`; Supabase sole implementation | Aurora/RDS remains design-only | Low/medium | tenant filter omission or response drift | exact query, tenant-negative, empty/error/provider tests pass | First read pilot complete; neighboring qualification paths unchanged |
...
| `tracepoint-attachments` flows, including Drill Library documents | typed server-only `ObjectStore` + Supabase Storage + `attachments`/`drill_documents` metadata | S3 remains design-only | High | service-role signing, object-key tenant escape, retained archive objects | path/upsert/signing/config tests plus drill upload/delete/view permission and tenant negatives; orphan reconciliation remains | Supabase adapter complete; Drill Library uses provider-neutral `drill-document` keys; no AWS provider |
