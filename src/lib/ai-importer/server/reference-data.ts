...
async function rows(result: PromiseLike<{ data: unknown[] | null; error: { message: string } | null }>) {
  const resolved = await result;
  if (resolved.error) throw new Error("Import reference data could not be loaded.");
...
export async function loadImportReferenceData(admin: any, departmentId: string, domain: ImportDomain): Promise<ImportReferenceData> {
  ...
  if (needsPeople) {
    const memberships = await rows(admin.from("department_memberships").select("user_id,badge_number,employee_number,rank_title,unit_name,is_active").eq("department_id", departmentId));
    const ids = memberships.map((membership) => String(membership.user_id));
    const profiles = ids.length ? await rows(admin.from("profiles").select("id,full_name,email,phone").in("id", ids)) : [];
  ...
  if (domain === "firearms") {
    [empty.firearms, empty.firearmAssignments] = await Promise.all([
      rows(admin.from("firearms").select("id,serial_number,asset_number,make,model,caliber,firearm_type,condition_status,acquisition_date,notes").eq("department_id", departmentId)),
      rows(admin.from("firearm_assignments").select("id,firearm_id,assigned_to_user_id,returned_at").eq("department_id", departmentId)),
    ]);
  }
  if (domain === "certifications") {
    [empty.certificationTypes, empty.certifications] = await Promise.all([
      rows(admin.from("certification_types").select("id,name,is_active,issuing_organization,expiration_required").eq("department_id", departmentId)),
      rows(admin.from("training_certifications").select("id,user_id,certification_type_id,credential_number,issue_date,expiration_date,issuing_organization,notes").eq("department_id", departmentId)),
    ]);
  }
  if (domain === "vehicles") {
    [empty.vehicles, empty.fleetEquipment] = await Promise.all([
      rows(admin.from("fleet_vehicles").select("*").eq("department_id", departmentId)),
      rows(admin.from("fleet_vehicle_equipment").select("id,vehicle_id,category,serial_number,tuning_fork_serial_number,warranty_expiration_date,status").eq("department_id", departmentId)),
    ]);
  }
  if (domain === "equipment") {
    [empty.equipmentTypes, empty.equipment] = await Promise.all([
      rows(admin.from("equipment_types").select("id,name,is_active,expiration_required").eq("department_id", departmentId)),
      rows(admin.from("equipment_assets").select("*").eq("department_id", departmentId)),
    ]);
  }
  return empty;
}
