...
export default function QualificationsPage() {
  ...
  useEffect(() => {
    ...
    async function loadQualificationRules() {
      try {
        const response = await fetch("/api/settings/current-rules", {
          method: "GET",
          cache: "no-store",
        });

        if (!response.ok) {
          throw new Error("Unable to load qualification rules.");
        }
        ...
        const validDays = Number(
          payload.rules?.qualification_valid_days,
        );

        const dueSoonDays = Number(
          payload.rules?.qualification_due_soon_days,
        );

        setQualificationValidDays(
          Number.isFinite(validDays) && validDays > 0
            ? validDays
            : DEFAULT_QUALIFICATION_VALID_DAYS,
        );

        setQualificationDueSoonDays(
