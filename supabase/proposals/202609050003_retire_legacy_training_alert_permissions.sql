-- INACTIVE PROPOSAL: do not place this file in supabase/migrations until the
-- legacy-permission audit is approved. It is intentionally not deployable by
-- the normal migration runner from this location.
--
-- The current application ignores these six catalog codes. This proposal does
-- not translate them into broader grants: doing so could silently grant full
-- analytics or agency-training management. Instead it snapshots every catalog,
-- global-default, and department-specific assignment before retiring the dead
-- rows. Existing public.audit_events rows are untouched.
