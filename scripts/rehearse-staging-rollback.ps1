[CmdletBinding()]
param(
 [Parameter(Mandatory)][Alias('CurrentImageTag')][ValidatePattern('^(?:[0-9a-f]{40}(?:-aws-native)?|sha256:[0-9a-f]{64})$')][string]$CurrentImageReference,
...
$separator=if($CurrentImageReference.StartsWith('sha256:')){'@'}else{':'}
if($current.status -ne 'ACTIVE' -or $current.containerDefinitions.Count -ne 1 -or $current.containerDefinitions[0].image -ne "559054714699.dkr.ecr.us-east-1.amazonaws.com/tracepoint-staging$separator$CurrentImageReference"){throw 'Current rollback baseline image mismatch.'}
$imageId=if($CurrentImageReference.StartsWith('sha256:')){"imageDigest=$CurrentImageReference"}else{"imageTag=$CurrentImageReference"}
...
try {
 ...
 [ordered]@{currentTaskDefinition=$currentArn;priorTaskDefinition=$PriorTaskDefinitionArn;imageReference=$CurrentImageReference;rollbackVerified=$rollbackVerified;rollbackSeconds=$rollbackSeconds;returnSeconds=$returnSeconds;correctedRuntimeRestored=$true}|ConvertTo-Json
}
