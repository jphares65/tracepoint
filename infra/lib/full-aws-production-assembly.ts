...
export function fullAwsProductionAssembly(app:cdk.App,input:FullAwsProductionTarget,offline:boolean){
 ...
 const imageBuild=new ImageBuildStack(app,"tracepoint-production-full-aws-image-build",{...common,repository:compute.repository,appSecrets:compute.awsNativeAppSecrets,providerMode:"aws-native",productionControls:true});imageBuild.addStackDependency(compute);
 const storage=new PrivateStorageStack(app,"tracepoint-production-full-aws-storage",{...common,taskRole:compute.taskRole,dataKey:security.dataKey});storage.addStackDependency(compute);storage.addStackDependency(security);
