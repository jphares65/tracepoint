...
'@ | Set-Content -LiteralPath (Join-Path $temporaryRoot 'invoke-tracepoint-staging-rollback.ps1')
 foreach($scenario in @('success','preflight','acceptance','evidence')) {
 ...
 Write-Host 'Passed four release orchestration cases: success, preflight denial, acceptance rollback, evidence rollback. Zero network calls.'
} finally {
