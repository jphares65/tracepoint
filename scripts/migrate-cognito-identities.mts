...
const [{ execFileSync }, { getPostgresPool }, { provisionExistingCognitoUser }] = await Promise.all([
...
for (const user of manifest.users) {
  ...
  const existing = await pool.query('select 1 from public.authentication_identity_links where provider=$1 and tracepoint_user_id=$2 and state in ($3,$4) limit 1', ['cognito', user.targetUserId, 'pending', 'active']);
...
await pool.end();
console.log(JSON.stringify({ manifestSha256: manifest.contentSha256, total: manifest.users.length, migrated, alreadyLinked, skipped, completed: completed.size, emailAddressesPrinted: false, activationLinksPrinted: false }));
