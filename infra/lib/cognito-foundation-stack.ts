...
export interface CognitoFoundationProps extends cdk.StackProps {
 environmentName:'staging'|'production';
 sesFromAddress:string;
 sesConfigurationSetName:string;
 taskRole?:iam.IRole;
 mobileClient?:{callbackUrl:string;logoutUrl:string};
}
export class CognitoFoundationStack extends cdk.Stack {
 readonly userPool:cognito.UserPool;
 readonly userPoolClient:cognito.UserPoolClient;
 readonly mobileUserPoolClient?:cognito.UserPoolClient;
 constructor(scope:Construct,id:string,props:CognitoFoundationProps){
  ...
  if(props.mobileClient){
   if(props.environmentName!=='staging'||props.mobileClient.callbackUrl!=='tracepoint://auth'||props.mobileClient.logoutUrl!=='tracepoint://logout')throw Error('Mobile Cognito client is restricted to the exact staging application scheme');
   const mobile=this.mobileUserPoolClient=pool.addClient('MobileApplication',{
    userPoolClientName:'tracepoint-staging-mobile',generateSecret:false,
    authFlows:{userSrp:true},preventUserExistenceErrors:true,enableTokenRevocation:true,
    accessTokenValidity:cdk.Duration.minutes(5),idTokenValidity:cdk.Duration.minutes(5),refreshTokenValidity:cdk.Duration.days(1),
    refreshTokenRotationGracePeriod:cdk.Duration.seconds(10),
    readAttributes:new cognito.ClientAttributes().withStandardAttributes({email:true,emailVerified:true}),
