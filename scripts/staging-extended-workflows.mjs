...
async function expectStatus(response,status,step){if(response.status()!==status){const body=await response.json().catch(()=>({}));const message=String(body.error??'');const known=['permission denied','permission is required','not enabled','violates check constraint','violates not-null constraint','violates foreign key constraint','Could not find the function','does not exist','invalid input syntax','PostgreSQL data operation failed','audit record could not be created','does not have a current passing inspection','does not have a current qualifying record'].find(x=>message.toLowerCase().includes(x.toLowerCase()))??'unclassified';const object=message.match(/(?:constraint|column) "([a-z0-9_]+)"/);const providerCode=/^[A-Z0-9_]{3,64}$/.test(String(body.code??''))?String(body.code):undefined;const readinessStatus=/^(Current|Due Soon|Overdue|Not Inspected|Failed|Not Required)$/.test(String(body.inspectionStatus??body.qualificationStatus??''))?String(body.inspectionStatus??body.qualificationStatus):undefined;console.log(JSON.stringify({acceptanceStep:step,status:response.status(),failureClass:known,providerCode,readinessStatus,structuralDiagnostic:/^(Could not|cannot|permission|new row|invalid|column|record|function|null value|relation|duplicate|insert or update|operator|structure|PostgreSQL|Fleet|The audit|Off-duty|This off-duty|This TracePoint module)/i.test(message)?message.replace(/"[^"]*"|'[^']*'/g,'[quoted]').replace(/[0-9a-f]{8}-[0-9a-f-]{27}/gi,'[id]').replace(/https?:\/\/\S+|\S+@\S+|[A-Za-z0-9_-]{32,}/g,'[value]').slice(0,160):undefined,schemaObject:object?.[1]}));}assert.equal(response.status(),status);}
...
export async function exerciseExtendedWorkflows({context,browser,baseURL,check,signIn}) {
 ...
 try {
  ...
  await check('off-duty submission, inspection, command approval and Inbox delivery',async()=>{
   ...
   const inspectionHistory=await context.request.get(endpoint+'/inspections');await expectStatus(inspectionHistory,200,'off-duty inspection history');const inspectionBody=await inspectionHistory.json();console.log(JSON.stringify({offDutyInspectionProbe:{count:Array.isArray(inspectionBody.inspections)?inspectionBody.inspections.length:0,result:inspectionBody.inspections?.[0]?.result,dateMatches:inspectionBody.inspections?.[0]?.inspectionDate===today}}));assert.equal(inspectionBody.inspections?.[0]?.result,'Pass');assert.equal(inspectionBody.inspections?.[0]?.inspectionDate,today);
  ...
  await check('fleet creation, inspection, report and permission isolation',async()=>{
   ...
   const inspected=await context.request.post('/api/fleet/vehicles/'+id+'/inspections',{data:{mileage:101,checklist:['body','tires','lights','controls','fluids','interior'].map(id=>({id,condition:'Pass'}))}});await expectStatus(inspected,201,'fleet inspection');
