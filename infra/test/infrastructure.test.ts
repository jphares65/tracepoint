...
test("production template retains resources, scales two to four tasks, and separates providers", () => {
  ...
  const runtime = new RuntimeStack(app,"production-runtime",{...props,vpc:network.vpc,repository:compute.repository,cluster:compute.cluster,appLogGroup:compute.appLogGroup,appSecrets:compute.appSecrets,executionRole:compute.executionRole,taskRole:compute.taskRole,certificateArn:"arn:aws:acm:us-east-1:111111111111:certificate/00000000-0000-4000-8000-000000000000",imageTag:"6b0e3028f3e5e97d567de20c05637bb0cb64e7b7",desiredCount:2,maxCapacity:4,deletionProtection:true,productionControls:true});
  const template = Template.fromStack(runtime);
