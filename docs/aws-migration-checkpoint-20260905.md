...
| Category | Weight | Complete | Exact remaining capability |
|---|---:|---:|---|
| AWS account/governance | 6 | 50.00% | production account authorized; organization controls validated |
| network and compute | 10 | 60.00% | production network deployed; production load/failure test |
| runtime deployment | 8 | 75.00% | production runtime deployed |
| database/schema/data migration | 16 | 62.50% | production data rehearsal; production database moved; production PITR restore timed |
| authentication | 10 | 60.00% | replacement identity provider deployed; MFA and session cutover validated |
| storage | 8 | 100.00% | Staging foundation complete; production objects remain unmoved. |
| email | 6 | 50.00% | SES application provider deployed; bounce/suppression delivery verified |
| DNS/TLS | 8 | 50.00% | production AWS certificate verified; production DNS cutover validated |
| CI/CD | 8 | 80.00% | OIDC release executed |
| security/monitoring/backups | 10 | 75.00% | notification escalation tested; production backup restore validated |
| production account readiness | 6 | 50.00% | dedicated production account authorized; exact live production role gate |
| cutover/rollback validation | 4 | 50.00% | DNS rollback rehearsed; agency cutover approved |
...
Latest pre-release measured September month-to-date: estimated **$1.3814978907**; billing lags. Monthly low-usage model **$68.42**, including disabled SES, or **$70.42** with the $2 bounded rehearsal reserve, below $75. The budget is an alert, not a spending cap. No NAT or permanent AWS database was added. Current measured evidence is refreshed separately.
