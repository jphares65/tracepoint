...
test('operations workflow cannot publish or deploy and restricts its OIDC session',()=>{
 ...
 assert.equal(credentials.with['allowed-account-ids'],'559054714699');assert.equal(credentials.with['managed-session-policies'],'arn:aws:iam::aws:ReadOnlyAccess');
 const policy=JSON.parse(credentials.with['inline-session-policy']);assert.equal(policy.Version,'2012-10-17');assert.equal(policy.Statement.length,2);
 assert.deepEqual(policy.Statement.map(x=>x.Action),['secretsmanager:GetSecretValue','kms:Decrypt']);
 for(const statement of policy.Statement){assert.equal(statement.Effect,'Allow');assert.ok(statement.Resource.includes(':us-east-1:559054714699:'));assert.equal(statement.Resource.includes('*'),false);}
 assert.equal(policy.Statement[1].Condition.StringEquals['kms:ViaService'],'secretsmanager.us-east-1.amazonaws.com');
