      ...
      <div className="mx-auto max-w-[1500px]">
        ...
        <main className="mt-6 rounded-[2rem] border border-slate-800 bg-slate-900/75 shadow-2xl shadow-black/20 backdrop-blur">
          ...
          {step === "interpret" && upload && sheet ? <section className="p-6 sm:p-8">
            ...
            <div className="mt-6 rounded-2xl border border-blue-900/60 bg-blue-950/20 p-5"><p className="text-sm font-bold text-blue-100">What TracePoint noticed</p><div className="mt-3 grid gap-3 text-xs leading-5 text-slate-300 md:grid-cols-2"><p><span className="font-bold text-slate-100">Likely identifiers:</span> {upload.interpretation.identifierColumns.join(", ") || "Review during mapping"}</p><p><span className="font-bold text-slate-100">Likely date formats:</span> {upload.interpretation.likelyDateFormats.join(", ") || "No date columns detected"}</p></div>{upload.interpretation.sheetAssessments.length > 1 ? <div className="mt-3 flex flex-wrap gap-2">{upload.interpretation.sheetAssessments.map((assessment) => <span key={assessment.sheetName} className="rounded-full border border-blue-900 px-2.5 py-1 text-[10px] text-blue-100">{assessment.sheetName}: {assessment.disposition.replaceAll("_", " ")} · {assessment.confidence}</span>)}</div> : null}{upload.interpretation.normalizationSuggestions.length ? <div className="mt-4 space-y-2">{upload.interpretation.normalizationSuggestions.map((suggestion, index) => <p key={`${suggestion.sourceColumn}-${index}`} className="rounded-xl border border-emerald-900/60 bg-emerald-950/20 p-3 text-xs text-emerald-100"><strong>Suggested fix:</strong> {suggestion.sourceValue} → {suggestion.suggestedValue} <span className="text-emerald-300/70">({suggestion.confidence}; review before applying)</span></p>)}</div> : null}</div>
