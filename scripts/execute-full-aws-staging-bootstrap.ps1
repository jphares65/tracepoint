...
if (-not $result -or $result.sourceMigrations -ne 76 -or $result.awsMigrations -ne 18 -or $result.runtimeRoleVerified -ne $true -or $result.supabaseAuthorizationReferences -ne 0) { throw 'Exact database bootstrap evidence was not produced.' }
...
Write-Host 'AWS-native staging database bootstrap completed with 76 source and 18 AWS compatibility migrations; no credential values were printed.'
