...
export async function exerciseRangeDocuments({context,browser,baseURL,department,check,signIn}){
 ...
 await check('Drill Library document upload, view, download, tenant denial and delete',async()=>{
  ...
  for(const [email,foreign] of [[process.env.TRACEPOINT_ACCEPTANCE_OFFICER_EMAIL,false],[process.env.TRACEPOINT_ACCEPTANCE_FOREIGN_EMAIL,true]]){
   const other=await browser.newContext({baseURL});try{const page=await other.newPage();const secret=foreign?process.env.TRACEPOINT_ACCEPTANCE_FOREIGN_TOTP_SECRET:process.env.TRACEPOINT_ACCEPTANCE_OFFICER_TOTP_SECRET;await signIn(page,email,process.env.TRACEPOINT_ACCEPTANCE_OFFICER_PASSWORD,secret);
