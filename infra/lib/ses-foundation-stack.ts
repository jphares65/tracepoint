...
export class SesFoundationStack extends cdk.Stack {
 ...
 constructor(scope:Construct,id:string,props:SesFoundationProps){
  ...
  const from=this.fromAddress='notifications@'+domain,mailFrom=props.mailFromSubdomain+'.'+domain;
  ...
  const identity=new ses.EmailIdentity(this,'SenderDomain',{
   ...
   mailFromDomain:mailFrom,mailFromBehaviorOnMxFailure:ses.MailFromBehaviorOnMxFailure.REJECT_MESSAGE,feedbackForwarding:false,
  ...
  const configArns=[configuration,cognitoConfiguration].map(item=>this.formatArn({service:'ses',resource:'configuration-set',resourceName:item.configurationSetName}));
  key.addToResourcePolicy(new iam.PolicyStatement({principals:[new iam.ServicePrincipal('ses.amazonaws.com')],actions:['kms:GenerateDataKey*','kms:Decrypt'],resources:['*'],conditions:{StringEquals:{'aws:SourceAccount':this.account,'aws:SourceArn':configArns}}}));
  ...
  cognitoConfiguration.addEventDestination('CognitoFeedbackEvents',{destination:ses.EventDestination.snsTopic(topic),events:[ses.EmailSendingEvent.BOUNCE,ses.EmailSendingEvent.COMPLAINT,ses.EmailSendingEvent.DELIVERY]});
