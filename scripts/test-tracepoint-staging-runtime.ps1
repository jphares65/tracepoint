...
$service = & aws.exe ecs describe-services --cluster tracepoint-staging --services tracepoint-staging --profile $Profile --region us-east-1 --output json 2>&1
...
if ($service.loadBalancers.Count) {
    ...
    $health = & aws.exe elbv2 describe-target-health --target-group-arn $targetArn --profile $Profile --region us-east-1 --output json 2>&1
...
$logs = & aws.exe logs describe-log-streams --log-group-name /tracepoint/staging/application --order-by LastEventTime --descending --limit 5 --profile $Profile --region us-east-1 --output json 2>&1
