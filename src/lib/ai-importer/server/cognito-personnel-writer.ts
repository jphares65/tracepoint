...
export type CognitoPersonnelWriteResult = {
  userId: string;
...
export async function writeCognitoPersonnel(
  input: Parameters<PersonnelImportWriter>[0] & { siteUrl: string },
): Promise<CognitoPersonnelWriteResult> {
  const { admin, departmentId, actorId, row, siteUrl } = input;
  ...
  if (row.action === "CREATE") {
    ...
    const invited = await inviteCognitoUser({
      actorUserId: actorId,
      departmentId,
      email: String(value.email),
      fullName,
    ...
    return {
      ...
      invitationSent: invited.invitationSent,
    };
  }
...
export function createCognitoPersonnelWriter(siteUrl: string): PersonnelImportWriter {
  return async (input) => {
    await writeCognitoPersonnel({ ...input, siteUrl });
  };
}
