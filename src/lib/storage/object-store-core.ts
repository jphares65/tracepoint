...
export type AttachmentDomain = "qualification" | "agency-training" | "firearm" | "fleet-inspection" | "drill-document";
...
const ATTACHMENT_POLICIES: Record<AttachmentDomain, { maxBytes: number; contentTypes?: ReadonlySet<string> }> = {
  ...
  "fleet-inspection": { maxBytes: 15 * 1024 * 1024, contentTypes: new Set(["application/pdf", "image/jpeg", "image/png", "image/webp"]) },
...
export function attachmentPathForUpload(domain: AttachmentDomain, input: AttachmentUploadInput, authorizedDepartmentId: string): AttachmentObjectPath {
  ...
  const fallback = domain === "qualification" ? "target-photo" : domain === "firearm" ? "attachment" : domain === "fleet-inspection" ? "evidence" : domain === "drill-document" ? "document" : "file";
