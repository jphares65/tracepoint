...
async function main(){
 ...
 try{
  ...
  const files=(await readdir('supabase/migrations')).filter(f=>/^\d+_.+\.sql$/.test(f)).sort();assert.equal(files.length,65);
  ...
  for(const file of ['validate-local-tenant-isolation.sql','validate-local-armory-workflows.sql'])await client.query(await readFile('scripts/'+file,'utf8'));
  ...
  const before=await snapshot(client);const dump=path.join(directory,'synthetic.dump');
  ...
  const run=promisify(execFile);await run('pg_dump',['-Fc','--no-owner','-f',dump,'tracepoint_rehearsal'],{env,timeout:120000});
  await client.query('create database tracepoint_restore');const started=Date.now();
  await run('pg_restore',['--no-owner','--exit-on-error','-d','tracepoint_restore',dump],{env,timeout:120000});
  ...
  for(const file of ['validate-local-tenant-isolation.sql','validate-local-armory-workflows.sql'])await restored.query(await readFile('scripts/'+file,'utf8'));
...
if(process.argv[1]?.endsWith('run-aws-postgres-rehearsal.mjs'))main().catch(error=>{console.error(JSON.stringify({rehearsal:'FAILED',errorName:error.name,sqlState:error.code}));process.exitCode=1;});
