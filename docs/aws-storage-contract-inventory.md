...
| Drill Library documents `/api/drill-library/[drillTemplateId]/documents`, `/api/drill-documents/[documentId]/*` | `resolveServerAccess`; the template key must exist in the selected department's pilot workspace. Active members may list/view/download. POST and DELETE require `manage_range_days`; every metadata lookup is department-scoped. | `tracepoint-attachments`; `{departmentId}/drill-document/{encoded template key}/{uuid}-{sanitized original name}`. PDF/JPG/PNG/WebP only, non-empty, 15 MB maximum, `upsert:false`. Department, prefix and object ID are server-derived. Inline view and download links expire after 60 seconds. | Dedicated `drill_documents` metadata records original filename, provider-neutral path, MIME, size, uploader and timestamps. Metadata failure compensates by removing the object. DELETE removes the object and metadata. Upload/delete attempt explicit `audit_events`. The mobile viewer displays images and embeds PDFs with open/download fallbacks. |
...
- The drill-document migration now idempotently defines `tracepoint-attachments`
  as private with a 25 MB bucket ceiling, preserving the largest established
  training-file flow. Each route continues to enforce its narrower type/size
  policy; object access remains server-only through service-role routes.
