...
Budget remains **$150/month**. Normal rolling deployment retains $5.62 of modeled
headroom; the max-storage rolling sensitivity exceeds the boundary by $3.58 and
is a no-go until cost is reduced or the owner separately authorizes a change.
...
`aws-cost-optimization-model-20260912.json`. Deployment is no-go unless the
budget remains exactly at the approved $150 amount.
...
1. **Spend:** authorize the production resources only while the deterministic steady and normal rolling projections remain within the unchanged $150/month Budget; no increase is authorized.
