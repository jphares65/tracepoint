import type { ArmoryReadDataSource, ArmoryResult } from "./read-repository-core.ts";
...
export class SupabaseArmoryReadDataSource implements ArmoryReadDataSource {
  private readonly db: ArmoryClient;
  constructor(db: ArmoryClient) { this.db = db; }
  ...
  listActiveMembers(id: string) { return this.db.from("department_memberships").select("user_id,rank_title,badge_number,unit_name").eq("department_id", id).eq("is_active", true); }
