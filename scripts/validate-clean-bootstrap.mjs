...
try {
  ...
  if (awsTargetOverlayCount) {
    ...
    try {
      ...
      const migrationOperationId = users.denied;
      const migrationSubject = "60000000-0000-4000-8000-000000000001";
      await runtimeClient.query("begin");
      await runtimeClient.query("set local role authenticated");
      await runtimeClient.query("select set_config('tracepoint.subject_id',$1,true)", [users.administrator]);
      await runtimeClient.query("select set_config('tracepoint.department_id',$1,true)", [departmentA]);
      await runtimeClient.query("select * from tracepoint_auth.prepare_existing_cognito_migration($1,$2,$3,$4)", [migrationOperationId, migrationOperationId, departmentA, users.denied]);
      assert.equal((await runtimeClient.query("select activation_status from public.department_memberships where department_id=$1 and user_id=$2", [departmentA, users.denied])).rows[0].activation_status, "pending_activation");
      await runtimeClient.query("commit");
      await runtimeClient.query("select tracepoint_auth.commit_existing_cognito_migration($1,$2,$3)", [migrationOperationId, migrationSubject, issuer]);
      await runtimeClient.query("select tracepoint_auth.finish_existing_cognito_migration($1,false,'activation_delivery_unconfirmed')", [migrationOperationId]);
      await runtimeClient.query("select tracepoint_auth.commit_existing_cognito_migration($1,$2,$3)", [migrationOperationId, migrationSubject, issuer]);
      await runtimeClient.query("select tracepoint_auth.finish_existing_cognito_migration($1,true,null)", [migrationOperationId]);
      assert.equal((await client.query("select activation_status from public.department_memberships where department_id=$1 and user_id=$2", [departmentA, users.denied])).rows[0].activation_status, "activation_sent");
      await assert.rejects(runtimeClient.query("select tracepoint_auth.finish_existing_cognito_migration($1,true,null)", [migrationOperationId]));
      await runtimeClient.query("begin");
      await runtimeClient.query("set local role authenticated");
