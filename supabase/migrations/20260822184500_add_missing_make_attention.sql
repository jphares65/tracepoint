...
declare
  ...
  v_attention_reasons text[] := array[]::text[];
...
begin
  ...
  if lower(trim(p_make)) in (
    ...
    v_attention_reasons :=
      array_append(v_attention_reasons, 'missing_make');
  end if;

  if lower(trim(p_model)) in (
    'tbd',
    'unknown',
    'tbd / unknown',
    'tbd/unknown'
    ...
    v_attention_reasons :=
      array_append(v_attention_reasons, 'missing_model');
  end if;

  if nullif(trim(p_caliber), '') is null
     or lower(trim(p_caliber)) in (
       'tbd',
       'unknown',
       'tbd / unknown',
       'tbd/unknown'
    ...
    v_attention_reasons :=
      array_append(v_attention_reasons, 'missing_caliber');
  end if;

  if (
    'missing_firearm_type' = any(coalesce(v_old.attention_reasons, array[]::text[]))
    and v_old.firearm_type::text = 'other'
    and p_firearm_type = 'other'
  ) then
    v_attention_reasons :=
      array_append(v_attention_reasons, 'missing_firearm_type');
  end if;

  v_needs_attention :=
    coalesce(array_length(v_attention_reasons, 1), 0) > 0;
  ...
  v_changed_fields := array_remove(array[
    ...
    case when v_old.needs_attention is distinct from v_needs_attention then 'needs_attention' end,
    case when v_old.attention_reasons is distinct from v_attention_reasons then 'attention_reasons' end
  ], null);
  ...
  set
    ...
    attention_reasons = v_attention_reasons,
    updated_at = now()
  where id = p_firearm_id
    and department_id = p_department_id
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'notes', v_old.notes,
      'needs_attention', v_old.needs_attention,
      'attention_reasons', v_old.attention_reasons
    ),
    jsonb_build_object(
      'make', v_new.make,
      'model', v_new.model,
      ...
      'notes', v_new.notes,
      'needs_attention', v_new.needs_attention,
      'attention_reasons', v_new.attention_reasons
    )
  );
