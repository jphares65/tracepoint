...
export class SupabaseRestLedgerRunnerStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: SupabaseRestLedgerRunnerStackProps) {
    ...
    if (props.vpcId !== 'vpc-0e73422a833ee8ac6' || props.publicSubnetIds.length !== 2 || new Set(props.publicSubnetIds).size !== 2 || !props.publicSubnetIds.every(subnet => ['subnet-0f4cbed3e60d90bfc', 'subnet-0a117bec5cb98607f'].includes(subnet))) throw new Error('Only the reviewed public migration subnets are permitted');
