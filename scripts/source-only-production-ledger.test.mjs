...
test("source secret validation permits exactly the reviewed direct, standard Pooler, and dedicated reader tuples", () => {
  ...
  const dedicatedReader = validateSourceSecret({ host: SESSION_POOLER_SOURCE_HOST, port: 5432, dbname: "postgres", username: "tracepoint_migration_reader.izlkwggluhlhzlumtzes", password: "redacted" });
  assert.equal(direct.endpointKind, "direct");
  assert.equal(pooler.endpointKind, "session-pooler");
  assert.equal(dedicatedReader.endpointKind, "session-pooler");
  for (const candidate of [
    { host: SESSION_POOLER_SOURCE_HOST, port: 5432, dbname: "postgres", username: "postgres.wrongprojectref000", password: "redacted" },
    { host: SESSION_POOLER_SOURCE_HOST, port: 5432, dbname: "postgres", username: "tracepoint_migration_reader.wrongprojectref000", password: "redacted" },
    { host: SESSION_POOLER_SOURCE_HOST, port: 5432, dbname: "postgres", username: "arbitrary_custom_user", password: "redacted" },
...
test("source-only SQL guard permits reads and rejects write, DDL, and target-write statements", () => {
  ...
  for (const sql of ["insert into public.profiles values (1)", "update public.profiles set email='x'", "create table public.forbidden(id int)", "create temporary table forbidden(id int)", "delete from public.profiles", "select pg_sleep(1); delete from public.profiles"]) assert.throws(() => assertSourceOnlySql(sql));
});

test("sanitized evidence excludes source credentials, identifiers, emails, and target-client state", () => {
  const evidence = createSanitizedEvidence({ capturedAtUtc: "2026-09-20T00:00:00.000Z", postgresVersion: "PostgreSQL test", snapshotId: "00000001-1", lsn: "0/1", source: { host: SESSION_POOLER_SOURCE_HOST, port: 5432, database: "postgres", endpointKind: "session-pooler" }, tables: [{ name: "profiles", kind: "table", rows: "96" }], migrationVersions: ["202601010001"], tenantDepartment: { departmentCount: 3, membershipCount: 95, membershipsMissingDepartment: 0, membershipsMissingProfile: 0 }, sourcePrivileges: { transactionEnforcedReadOnly: true, databaseCreatePrivilege: false, publicTablesWithWritePrivilege: 0 } });
...
test("runner imports only the source database and Secrets Manager clients", async () => {
  ...
  assert.doesNotMatch(source, /create\s+(?:temporary\s+|temp\s+)?table/i);
