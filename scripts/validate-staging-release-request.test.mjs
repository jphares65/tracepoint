...
const context={branch:'refs/heads/main',head:'b'.repeat(40),parent:'a'.repeat(40),changedFiles:['.github/staging-release.json']};
...
test('production targets, unreviewed changes and feature-branch releases are rejected',()=>{
 ...
 for(const patch of [{branch:'refs/heads/codex/aws-staging-readiness-20260902'},{changedFiles:['.github/staging-release.json','src/app/page.tsx']},{changedFiles:[]}])assert.throws(()=>validateReleaseRequest(request,{...context,...patch}));
});
...
test('existing immutable image requires ancestor and identical runtime source',()=>{
 ...
 const context={branch:'refs/heads/main',head:'c'.repeat(40),parent:request.reviewedCommit,changedFiles:['.github/staging-release.json'],imageIsAncestor:true,runtimeSourceMatches:true};
...
test('rollback request is confined to the exact staging task family',()=>{
 ...
 const context={branch:'refs/heads/main',head:'c'.repeat(40),parent:request.reviewedCommit,changedFiles:['.github/staging-release.json']};assert.equal(validateReleaseRequest(request,context).rollbackPriorTaskArn,request.rollbackPriorTaskArn);
