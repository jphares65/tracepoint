...
test('production assembly retains provider isolation and two-to-four capacity',()=>{
 ...
 runtime.hasResourceProperties('AWS::ECS::TaskDefinition',{ContainerDefinitions:Match.arrayWith([Match.objectLike({Environment:Match.arrayWith([{Name:'TRACEPOINT_FROM_EMAIL',Value:'contact@tracepointhq.com'},{Name:'TRACEPOINT_EMAIL_PROVIDER',Value:'brevo'},{Name:'TRACEPOINT_STORAGE_PROVIDER',Value:'supabase'}])})])});
 ...
 alerts.hasResourceProperties('AWS::CloudWatch::CompositeAlarm',{AlarmName:'tracepoint-production-runtime-alert'});alerts.hasResourceProperties('AWS::SNS::Subscription',{Protocol:'email',Endpoint:'contact@tracepointhq.com'});alerts.resourceCountIs('AWS::SNS::Subscription',2);alerts.resourceCountIs('AWS::SQS::Queue',2);
