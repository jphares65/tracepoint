...
import { productionRestLedgerArchivePaths, validateCleanProductionScan, validateProductionRestLedgerArchive } from './production-publication-core.mjs';
...
const directory=mkdtempSync(join(tmpdir(),'tracepoint-rest-ledger-')),archive=join(directory,'source.zip');
try{
 command('git.exe',['archive','--format=zip',`--output=${archive}`,commit,'--',...productionRestLedgerArchivePaths]);
 const entries=command('tar.exe',['-tf',archive]).split(/\r?\n/).filter(entry=>!entry.endsWith('/'));
 const tracked=new Set(command('git.exe',['ls-tree','-r','--name-only',commit]).split(/\r?\n/));validateProductionRestLedgerArchive(entries,tracked);
 ...
 if(existing.images?.length===1){const scan=aws(['ecr','describe-image-scan-findings','--repository-name','tracepoint-production','--image-id',`imageTag=${tag}`]);validateCleanProductionScan(scan);console.log(JSON.stringify({existingImmutableImage:true,sourceCommit:commit,imageTag:tag,imageDigest:existing.images[0].imageId.imageDigest,cleanScan:true}));}
  ...
  const scan=aws(['ecr','describe-image-scan-findings','--repository-name','tracepoint-production','--image-id',`imageTag=${tag}`]);validateCleanProductionScan(scan);assert.match(scan.imageId.imageDigest,/^sha256:[0-9a-f]{64}$/);console.log(JSON.stringify({imagePublished:true,sourceCommit:commit,imageTag:tag,imageDigest:scan.imageId.imageDigest,cleanScan:true,sourceOnly:true}));
 }
