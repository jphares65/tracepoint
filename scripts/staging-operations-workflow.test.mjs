...
test('operations workflow cannot publish or deploy and restricts its OIDC session',()=>{
 ...
 const policy=JSON.parse(credentials.with['inline-session-policy']);assert.equal(policy.Version,'2012-10-17');assert.equal(policy.Statement.length,2);assert.ok(JSON.stringify(policy).length<=850);
 ...
 assert.deepEqual(policy.Statement[1],{Effect:'Allow',Action:['organizations:DescribeOrganization','organizations:ListAccounts','organizations:ListPoliciesForTarget'],Resource:'*'});
