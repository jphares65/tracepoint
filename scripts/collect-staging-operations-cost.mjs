...
try {
 ...
 const model=JSON.parse(readFileSync(new URL('../docs/aws-native-staging-cost-model-20260910.json',import.meta.url),'utf8'));
 const modeledMonthlyUSD=Object.values(model.componentsCents).reduce((sum,value)=>sum+value,0)/100;assert.equal(modeledMonthlyUSD,model.projectedTotalCents/100);
 ...
 const withinCeiling=actual<125&&modeledMonthlyUSD<=125&&(!costExplorer.available||costExplorer.unblendedCostUSD<125);
 console.log(JSON.stringify({account,region,queriedAtUTC:now.toISOString(),period:{start,endExclusive:end},budgetActualUSD:actual,budgetLimitUSD:125,modeledMonthlyUSD,projectedHeadroomUSD:model.headroomCents/100,costExplorer,withinCeiling,billingLagNote:'Budget actual and estimated billing lag usage. This model is not a measured monthly bill or hard spending cap.'},null,2));
