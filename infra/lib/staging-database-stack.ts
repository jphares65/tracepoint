...
export function validateStagingDatabaseLease(props: Pick<StagingDatabaseStackProps, "expiresAfterUtc" | "leaseOwner" | "leaseReference">, now = Date.now()) {
  const expires = Date.parse(props.expiresAfterUtc);
  if (!Number.isFinite(expires) || expires <= now + cdk.Duration.minutes(30).toMilliseconds() || expires > now + cdk.Duration.days(7).toMilliseconds()) throw new Error("Staging database lease must expire between 30 minutes and 7 days from synthesis");
  if (!/^[A-Za-z0-9][A-Za-z0-9 .@_-]{2,79}$/.test(props.leaseOwner) || !/^[A-Za-z0-9][A-Za-z0-9._:/-]{2,159}$/.test(props.leaseReference)) throw new Error("Explicit staging database lease owner and approval reference required");
