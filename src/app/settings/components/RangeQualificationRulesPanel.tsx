...
export default function RangeQualificationRulesPanel({
  ...
  function patchAnalyticsDashboard<
    K extends keyof AnalyticsDashboardConfiguration,
  >(key: K, value: AnalyticsDashboardConfiguration[K]) {
    setRules((current) => ({
      ...current,
      analytics_dashboard: {
        ...current.analytics_dashboard,
        [key]: value,
      },
    }));
  }
      ...
      <div className="grid gap-5 p-5 lg:grid-cols-2">
        ...
        <div className="space-y-4 lg:col-span-2">
          <div className="rounded-xl border border-blue-800/60 bg-blue-950/20 p-4">
            ...
            <p className="mt-1 max-w-4xl text-xs leading-5 text-blue-100/60">
              Choose which range and qualification insights appear for this
              agency. These controls change presentation and signal thresholds;
              they do not change permissions, source records, or enabled modules.
            </p>
          </div>
            ...
            <div className="mt-2 grid gap-3 lg:grid-cols-2">
              {[
                ...
                <Toggle
                  ...
                  checked={rules.analytics_dashboard.command_dashboard_cards[key as keyof AnalyticsDashboardConfiguration["command_dashboard_cards"]]}
                  disabled={!canAdminister}
                  onChange={(value) =>
                    patchAnalyticsDashboard("command_dashboard_cards", {
                      ...rules.analytics_dashboard.command_dashboard_cards,
                      [key]: value,
                    })
                  }
                />
            ...
            <div className="mt-2 grid gap-3 lg:grid-cols-2">
              {[
                ...
                <Toggle
                  ...
                  title={title}
                  description={description}
                  checked={rules.analytics_dashboard.command_dashboard_sections[key as keyof AnalyticsDashboardConfiguration["command_dashboard_sections"]]}
                  disabled={!canAdminister}
                  onChange={(value) =>
                    patchAnalyticsDashboard("command_dashboard_sections", {
                      ...rules.analytics_dashboard.command_dashboard_sections,
                      [key]: value,
                    })
                  }
                />
            ...
            <div className="mt-2 grid gap-3 lg:grid-cols-2">
              {[
                ...
                <Toggle
                  ...
                  title={title}
                  description={description}
                  checked={rules.analytics_dashboard.analytics_metrics[key as keyof AnalyticsDashboardConfiguration["analytics_metrics"]]}
                  disabled={!canAdminister}
                  onChange={(value) =>
                    patchAnalyticsDashboard("analytics_metrics", {
                      ...rules.analytics_dashboard.analytics_metrics,
                      [key]: value,
                    })
                  }
                />
            ...
            <div className="mt-2 grid gap-3 lg:grid-cols-2">
              {[
                ...
                <Toggle
                  ...
                  title={title}
                  description={description}
                  checked={rules.analytics_dashboard.analytics_sections[key as keyof AnalyticsDashboardConfiguration["analytics_sections"]]}
                  disabled={!canAdminister}
                  onChange={(value) =>
                    patchAnalyticsDashboard("analytics_sections", {
                      ...rules.analytics_dashboard.analytics_sections,
                      [key]: value,
                    })
                  }
                />
          ...
          <div className="grid gap-3 lg:grid-cols-3">
            <QuantityInput
              ...
              unit="points"
              value={rules.analytics_dashboard.trend_change_threshold}
              min={0}
              max={100}
              disabled={!canAdminister}
              onChange={(value) => patchAnalyticsDashboard("trend_change_threshold", value)}
            />
            <QuantityInput
              label="Repeated deficiency threshold"
              description="Number of range days with a failed result, observed deficiency, or remediation recommendation before the historical pattern independently becomes high priority. A latest failed or remediation-recommended result remains high priority."
              unit="range days"
              value={rules.analytics_dashboard.repeated_deficiency_count}
              min={1}
              max={20}
              disabled={!canAdminister}
              onChange={(value) => patchAnalyticsDashboard("repeated_deficiency_count", value)}
            />
            <QuantityInput
              label="Attention item limit"
              description="Maximum number of items shown in the Command Dashboard attention list."
              unit="items"
              value={rules.analytics_dashboard.command_attention_item_limit}
              min={1}
              max={25}
              disabled={!canAdminister}
