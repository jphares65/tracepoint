...
| Shared attachment download `GET /api/attachments/[attachmentId]/download` | `resolveServerAccess`; metadata query requires matching `attachmentId`, selected `department_id`, and `archived_at is null`. There is no entity-specific feature or permission check. Query error is 500, absent/foreign/archived metadata is 404. | Reads the stored path from DB and signs that path in `tracepoint-attachments` for exactly 60 seconds with the stored original filename as download disposition. The browser controls only the attachment ID; bucket and path come from server metadata. | Signing failure or missing URL is 500. Success is a Next.js redirect to the signed Supabase URL. No download audit is recorded. A malformed or cross-tenant path in otherwise department-scoped metadata is not prefix-validated before service-role signing. |
