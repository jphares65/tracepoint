...
| `/api/notifications/email-dispatch` | DB queue + shared secret + Brevo | Provider boundary; Brevo retained, SES alternative | Medium | forged dispatch/replay, PII logs | auth failure, retry/status parity, provider error mapping | Email contract first |
...
| `tracepoint-attachments` flows | Supabase Storage + attachments metadata | S3 SSE-KMS + presigned URLs | High | object-key tenant escape | upload/download/delete, MIME/size, scan state, orphan reconcile | Storage adapter, bucket later |
| Department patch | `department-assets` bucket | S3 SSE-KMS | Medium | public asset/path injection | image validation, replacement rollback | After attachment pattern |
