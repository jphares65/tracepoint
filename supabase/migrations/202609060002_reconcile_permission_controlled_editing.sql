...
-- Historical staging created this relationship against profiles while the
-- production-applied migration created it against auth.users. Both identify
-- the same durable user UUID. Converge the constraint without rewriting rows.
alter table public.pilot_range_workspaces
  drop constraint if exists pilot_range_workspaces_updated_by_user_id_fkey;

alter table public.pilot_range_workspaces
  add constraint pilot_range_workspaces_updated_by_user_id_fkey
