...
try {
 ...
 os.userInfo=()=>({uid:-1,gid:-1,username:'local',homedir:os.tmpdir(),shell:null});
...
before(async()=>{
 directory=await mkdtemp(path.join(os.tmpdir(),'tracepoint-manifest-test-'));const port=await localPostgresPort();
 server=new EmbeddedPostgres({databaseDir:directory,user:'postgres',password:'local-test-only',port,persistent:true,postgresFlags:['-h','127.0.0.1'],initdbFlags:['--encoding=UTF8','--locale=C'],onLog:message=>{startupDiagnostics.push(message)},onError:()=>{}});
...
after(async()=>{
 if(process.platform==='win32'&&server?.process){
