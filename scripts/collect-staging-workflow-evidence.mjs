 ...
 if(!run){console.log(JSON.stringify({commit:sha,workflowFound:false,preview:preview?.state??'pending'}));}
  ...
  if(process.argv.includes('--completed-logs')){
   ...
   for(const job of jobs.jobs){
    ...
    const evidence={job:job.name,testCounts:[...logs.matchAll(/(?:#|\u2139) tests (\d+)/g)].map(x=>Number(x[1])),fixtures:[],deliveries:[],brevoPrerequisites:[],structuralGates:[],resourceDiff:[...new Set(logs.match(/\[(?:~|\+|-)\] AWS::[A-Za-z0-9:]+[^\r\n]*/g)??[])]};
    if(operations)evidence.operations=operationsEvidence(logs);
    for(const line of logs.split('\n')){try{const value=JSON.parse(line.trim());if(value.fixtureRun){const item={run:value.fixtureRun};for(const key of ['cleanup','storageCleanup','auditCreation','custodyHistory','documentAudit'])if(value[key]!==undefined)item[key]=value[key];if(Object.keys(item).length>1)evidence.fixtures.push(item);}if(value.submitted===true&&value.recipient==='verified Brevo account owner')evidence.deliveries.push({run:value.run,messageId:value.messageId,submitted:true,recipient:value.recipient,taskRevision:value.taskRevision});if(['pending','verified'].includes(value.delivery))evidence.deliveries.push({run:value.run,messageId:value.messageId,delivery:value.delivery,events:Array.isArray(value.events)?value.events.map(x=>({event:x.event,date:x.date})):[]});if(value.brevoPrerequisites==='verified')evidence.brevoPrerequisites.push({status:'verified',taskRevision:value.taskRevision,sender:value.sender,recipient:value.recipient});if(value.brevoDelivery==='failed'&&['identity','secret','task-configuration','account-metadata','sender-metadata','submission','delivery-events'].includes(value.stage))evidence.brevoPrerequisites.push({status:'failed',stage:value.stage,submitted:value.submitted===true,sensitiveDetailsPrinted:value.sensitiveDetailsPrinted===true});if(value.safe===true&&typeof value.scope==='string')evidence.structuralGates.push({safe:true,scope:value.scope});}catch{}}
