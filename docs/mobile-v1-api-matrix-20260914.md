...
| Attendance | `POST /api/mobile/range-days/{rangeDayId}` with `attendance` or `bulk-attendance` | `manage_range_days` or department administration | Individual or mark-all-present; signed operation ID; idempotent replay; optimistic workspace revision |
...
| Score save/resume | same endpoint with `save-score` | `score_range_days`, `manage_qualifications`, or range management | Server calculates threshold-based pass/fail; per-result idempotency, encrypted local draft/retry state, optimistic revision |
