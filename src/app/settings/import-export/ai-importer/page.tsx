...
import { AlertTriangle, ArrowLeft, ArrowRight, Check, CheckCircle2, Download, FileCheck2, Loader2, LockKeyhole, RotateCcw, ShieldCheck, Sparkles, Upload } from "lucide-react";
...
import { IMPORT_DOMAINS, type ColumnMapping, type FileMetadata, type ImportDomain, type ImportExecutionResult, type ImportInterpretation, type ImportPayload, type ImportPreview, type ParsedSheet } from "@/lib/ai-importer/types";
...
export default function AiImporterPage() {
  ...
  const payload = useMemo<ImportPayload | null>(() => upload && sheet ? { file: upload.file, domain, sheetName: sheet.name, headerRow, matrix: sheet.matrix, mappings } : null, [upload, sheet, domain, headerRow, mappings]);

  function reset() {
    setStep("upload"); setUpload(null); setPreview(null); setResult(null); setError("");
  ...
  function reconfigure(nextDomain: ImportDomain, nextSheetName: string, nextHeader: number) {
    ...
    setMappings(suggestMappings(nextSheet, nextHeader, nextDomain)); setPreview(null);
  }
  ...
  async function chooseFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setUpload(data); setDomain(data.interpretation.domain); setSheetName(data.interpretation.sheetName); setHeaderRow(data.interpretation.headerRow); setMappings(data.interpretation.mappings); setStep("interpret");
  ...
  async function runPreview() {
    ...
    try {
      const response = await fetch("/api/settings/ai-importer/preview", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload), credentials: "same-origin" });
      if (!response.ok) throw new Error(await errorMessage(response));
      setPreview(await response.json() as ImportPreview); setStep("validate");
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(37,99,235,0.16),transparent_36%),linear-gradient(180deg,#020617,#07111f)] px-4 py-8 text-slate-100 sm:px-7 lg:px-10">
      <div className="mx-auto max-w-[1500px]">
        ...
        <main className="mt-6 rounded-[2rem] border border-slate-800 bg-slate-900/75 shadow-2xl shadow-black/20 backdrop-blur">
          ...
          {step === "validate" && preview ? <section className="p-6 sm:p-8"><div className="border-b border-slate-800 pb-5"><p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-300">Validation preview</p><h2 className="mt-2 text-2xl font-black text-white">Every row has a proposed action</h2><p className="mt-2 text-sm text-slate-400">No database writes occurred. Blocked rows must be resolved before import.</p></div>
            ...
            <div className="mt-5 max-h-[540px] overflow-auto rounded-2xl border border-slate-800"><table className="w-full min-w-[900px] text-left text-xs"><thead className="sticky top-0 bg-slate-950 text-[10px] uppercase tracking-wider text-slate-500"><tr><th className="p-3">Row</th><th className="p-3">Action</th><th className="p-3">Status</th><th className="p-3">Matched by</th><th className="p-3">Changes / reasons</th></tr></thead><tbody className="divide-y divide-slate-800">{preview.rows.map((row) => <tr key={row.rowNumber} className="bg-slate-900/70 align-top"><td className="p-3 font-bold text-white">{row.rowNumber}</td><td className="p-3"><span className="font-black text-blue-200">{row.action}</span></td><td className="p-3 capitalize text-slate-300">{row.status}</td><td className="p-3 text-slate-400">{row.matchReason || "—"}</td><td className="p-3 text-slate-300">{row.issues.length ? <ul className="space-y-1">{row.issues.map((issue, index) => <li key={index} className={issue.severity === "error" ? "text-red-300" : "text-amber-200"}>{issue.message}</li>)}</ul> : row.changes.length ? <ul className="space-y-1">{row.changes.map((change) => <li key={change.field}>{change.label}: <span className="text-slate-500">{String(change.previous ?? "blank")}</span> → {String(change.next ?? "blank")}</li>)}</ul> : "No changes"}</td></tr>)}</tbody></table></div>
          ...
          {step === "review" && preview ? <section className="p-6 sm:p-8"><div className="mx-auto max-w-3xl"><div className="text-center"><ShieldCheck className="mx-auto h-12 w-12 text-blue-300"/><p className="mt-4 text-xs font-bold uppercase tracking-[0.2em] text-blue-300">Administrator review</p><h2 className="mt-2 text-2xl font-black text-white">Approve each decision boundary</h2><p className="mt-2 text-sm text-slate-400">This approval is bound to this file hash, worksheet, header row, mappings, current agency, actor, and validation result.</p></div>
              <div className="mt-7 space-y-3">{([
                ...
                ["validation", `I reviewed ${preview.summary.create} creates, ${preview.summary.update} updates, ${preview.summary.skip} skips, and ${preview.summary.warnings} warning rows.`],
