...
try {
 ...
 const budget=aws(['budgets','describe-budget','--account-id',account,'--budget-name','tracepoint-staging-monthly-75']).Budget;
 assert.equal(budget.BudgetLimit.Unit,'USD');assert.equal(Number(budget.BudgetLimit.Amount),75);
 ...
 const withinCeiling=actual<75&&modeledMonthlyUSD+2<75&&(!costExplorer.available||costExplorer.unblendedCostUSD<75);
 console.log(JSON.stringify({account,region,queriedAtUTC:now.toISOString(),period:{start,endExclusive:end},budgetActualUSD:actual,budgetLimitUSD:75,modeledMonthlyUSD,disposableRehearsalReserveUSD:2,costExplorer,withinCeiling,billingLagNote:'Budget actual and estimated billing lag usage. This model is not a measured monthly bill or hard spending cap.'},null,2));
