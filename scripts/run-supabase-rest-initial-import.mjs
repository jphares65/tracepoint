...
async function runDatabase() {
  ...
  try { const snapshot = await sourceSnapshot(); phase = "target TLS preflight"; await client.connect(); const preflight = await preflightTarget(client, snapshot); phase = "proven migration-artifact cleanup"; const cleanup = await cleanupProvenMigrationArtifacts(client, snapshot, preflight); const results = []; phase = "audit prerequisites: departments"; results.push(await importRelation(client, "departments", snapshot.rows.get("departments") ?? [], preflight.mappings.find(mapping => mapping.relation === "departments"), preflight.resumePlan.get("departments"))); phase = "audit prerequisites: identity anchors"; await insertIdentityAnchors(client, snapshot.users); for (const relation of ["audit_events", "retired_permission_assignment_audit"]) assert.equal(Number((await client.query(`select count(*)::int as count from public.${relation}`)).rows[0].count), 0, `AUDIT_NOT_EMPTY_BEFORE_SOURCE_HISTORY:${relation}`); phase = "source audit history"; for (const relation of AUDIT_HISTORY_RELATIONS) results.push(await importRelation(client, relation, snapshot.rows.get(relation) ?? [], preflight.mappings.find(mapping => mapping.relation === relation), preflight.resumePlan.get(relation))); phase = "audit identity sequence repair"; const auditIdentitySequences = await repairSequences(client); phase = "relational import"; for (const item of preflight.order) { if (["departments", ...AUDIT_HISTORY_RELATIONS].includes(item)) continue; phase = `relational import:${item}`; if (item === "profiles") results.push(await hydrateMigrationAnchorProfiles(client, snapshot, preflight)); else if (item === NULLABLE_TRAINING_CERTIFICATION_CYCLE.token) results.push(await importNullableTrainingCertificationCycle(client, snapshot, preflight)); else results.push(await importRelation(client, item, snapshot.rows.get(item) ?? [], preflight.mappings.find(mapping => mapping.relation === item), preflight.resumePlan.get(item))); } phase = "target sequence repair"; const identitySequences = await repairSequences(client); phase = "target reconciliation"; const evidence = await verifyDatabase(client, snapshot, preflight); console.log(JSON.stringify({ status: "PASSED", runId: RUN_ID, authorizationReference: AUTHORIZATION_REFERENCE, mode, sourceReadOnly: true, targetWriteScope: "approved-initial-import", artifactCleanup: cleanup, importedRelations: results, auditIdentitySequences, identityPreservation: [...preflight.identityPreservation.values()], identitySequences, evidence, targetClientsInitialized: true, cognitoClientsInitialized: false })); }
