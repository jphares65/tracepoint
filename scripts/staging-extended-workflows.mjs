...
async function expectStatus(response,status,step){if(response.status()!==status){const body=await response.json().catch(()=>({}));const message=String(body.error??'');const known=['permission denied','permission is required','not enabled','violates check constraint','violates not-null constraint','violates foreign key constraint','Could not find the function','does not exist','invalid input syntax','PostgreSQL data operation failed','audit record could not be created'].find(x=>message.toLowerCase().includes(x.toLowerCase()))??'unclassified';const object=message.match(/(?:constraint|column) "([a-z0-9_]+)"/);const providerCode=/^[A-Z0-9_]{3,64}$/.test(String(body.code??''))?String(body.code):undefined;console.log(JSON.stringify({acceptanceStep:step,status:response.status(),failureClass:known,providerCode,structuralDiagnostic:/^(Could not|cannot|permission|new row|invalid|column|record|function|null value|relation|duplicate|insert or update|operator|structure|PostgreSQL|Fleet|The audit|Off-duty|This TracePoint module)/i.test(message)?message.replace(/"[^"]*"|'[^']*'/g,'[quoted]').replace(/[0-9a-f]{8}-[0-9a-f-]{27}/gi,'[id]').replace(/https?:\/\/\S+|\S+@\S+|[A-Za-z0-9_-]{32,}/g,'[value]').slice(0,160):undefined,schemaObject:object?.[1]}));}assert.equal(response.status(),status);}
